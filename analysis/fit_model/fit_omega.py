...
def main(subject, sourcedata, trialwise, masks, smoothed, log_space, progressbar,
    ...
    for mask_str in masks:
        ...
        def load_parameters():
            ...
            if mask_str[-2:] in ['_L', '_R']:
                for par in parameter_labels:
                    fn = op.join(derivatives, f'modelfit2{trial_ext}_surf{smooth_ext}',
                                 f'sub-{subject}', 'func',
                                 f'sub-{subject}_space-fsaverage6_desc-{par}.optim_hemi-{mask_str[-1]}.func.gii')
                ...
                for par in parameter_labels:
                    fn_l = op.join(derivatives, f'modelfit2{trial_ext}_surf{smooth_ext}',
                                 f'sub-{subject}', 'func',
                                 f'sub-{subject}_space-fsaverage6_desc-{par}.optim_hemi-L.func.gii')
                    par_l = surface.load_surf_data(fn_l)

                    fn_r = op.join(derivatives, f'modelfit2{trial_ext}_surf{smooth_ext}',
                                 f'sub-{subject}', 'func',
                                 f'sub-{subject}_space-fsaverage6_desc-{par}.optim_hemi-R.func.gii')
        ...
        if log_space:
            paradigm = d.index.get_level_values('log(stimulus)')
        else:
            paradigm = d.index.get_level_values('stimulus')

        paradigm.index = d.index

        model = GaussianPRF(paradigm=paradigm,
                            data=d, parameters=parameters)
        pseudoWWT = model.init_pseudoWWT(stimulus_range, parameters)

        predictions = model.predict()
        predictions.index = d.index

        residuals = d - predictions

        residual_fitter = ResidualFitter(
            model, d.values, paradigm, parameters)
        ...
        target_dir = op.join(
            derivatives, f'omega{trialwise_ext}{smooth_ext}{distance_ext}', f'sub-{subject}', 'func')

        if not op.exists(target_dir):
            os.makedirs(target_dir)

        
        np.savetxt(op.join(target_dir, f'sub-{subject}_mask-{mask_str}_omega.tsv'), omega)
